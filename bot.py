...
def get_user(user_id: int) -> dict:
    if user_id not in user_data:
        user_data[user_id] = {
            ...
            "notes": [],
            "plan": [],
            "mode": "chat"  # chat | add_task | add_note | add_plan
        }
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Могу помочь с задачами, заметками, планированием дня "
        "и просто поговорить. Выбери что хочешь:",
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• /notes — заметки\n"
        "• /plan — план дня\n"
        "• Просто пиши — я отвечу как ИИ-ассистент\n\n"
        "В меню нажимай кнопки для управления.",
        parse_mode="Markdown"
    )
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # TASKS
        ...
        text = f"✅ *Задачи*\n\nАктивных: {len(active)} | Выполнено: {len(done)}\n"
        if active:
            text += "\n*Активные:*\n" + "\n".join(f"• {t['text']}" for t in active[:10])
        else:
            text += "\nНет активных задач."
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=tasks_keyboard(data["tasks"]))

    elif cb == "add_task":
        data["mode"] = "add_task"
        await query.edit_message_text(
            "✏️ Напиши задачу (можно несколько через Enter):",
            reply_markup=back_keyboard()
        )
        ...
        for t in data["tasks"]:
            if str(t["id"]) == tid:
                ...
                break
        active = [t for t in data["tasks"] if not t["done"]]
        done_count = len([t for t in data["tasks"] if t["done"]])
        text = f"✅ *Задачи*\n\nАктивных: {len(active)} | Выполнено: {done_count}"
        if active:
            text += "\n\n*Активные:*\n" + "\n".join(f"• {t['text']}" for t in active[:10])
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=tasks_keyboard(data["tasks"]))
    ...
    # NOTES
        ...
        data["mode"] = "add_note"
        await query.edit_message_text(
            "✏️ Напиши заметку (первая строка — заголовок):",
            reply_markup=back_keyboard()
        )
        ...
        for i, n in enumerate(data["notes"][-10:], 1):
            text += f"*{i}. {n['title']}*\n{n['body'][:100]}{'…' if len(n['body']) > 100 else ''}\n\n"
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=notes_keyboard())

    # PLAN
    elif cb == "menu_plan":
        await query.edit_message_text(
            f"📅 *План дня*\n\nЗаписей: {len(data['plan'])}",
            parse_mode="Markdown",
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "add_plan":
        ...
        if len(parts) == 2 and ":" in parts[0]:
            time_str, task_text = parts[0], parts[1]
            data["plan"].append({"time": time_str, "text": task_text})
            data["mode"] = "chat"
            await update.message.reply_text(
                f"📅 Добавлено в план:\n`{time_str}` — {task_text}",
                parse_mode="Markdown",
            ...
            await update.message.reply_text(
                "⚠️ Формат: `10:00 Встреча с командой`",
                parse_mode="Markdown"
            )
        return
    ...
    if active_tasks:
        tasks_ctx = f"\nАктивные задачи пользователя: {', '.join(t['text'] for t in active_tasks[:5])}"

    plan_ctx = ""
    if data["plan"]:
        sorted_plan = sorted(data["plan"], key=lambda x: x["time"])
        plan_ctx = f"\nПлан дня: {', '.join(p['time'] + ' ' + p['text'] for p in sorted_plan[:5])}"

    system = (
        "Ты — дружелюбный личный ассистент в Telegram. "
        "Отвечай по-русски, кратко и по делу. "
        "Не используй markdown (никаких *, _, ` и т.д.) — только простой текст. "
        "Будь тёплым и полезным."
        + tasks_ctx + plan_ctx
    )
